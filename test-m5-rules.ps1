# M5 service-booking date/time/amount validation test
...
# R2a: negative amount rejected on create
...
# R2b: update negative amount rejected
if ($sbId) {
...
# R2c: zero amount accepted
$r = Try-Api 'Put' "/service-bookings/$sbId" @{ amount=0 }
T "R2c zero amount accepted" ($r.ok) $(if ($r.ok) {"updated"} else {$r.msg})

# R3a: end < start rejected
...
# R3d: update end<start rejected (partial update, existing startTime=10:00)
if ($sbId) {
...
# R3e: invalid time format rejected
